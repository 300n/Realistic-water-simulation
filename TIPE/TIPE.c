        ...
        for (int j = SampleCell.y - 1; j <= SampleCell.y + 1; j++) {
            ...
            if (key>=0 && key<particle_grid.MATlength*particle_grid.MATwidth) {
                ...
                for (int l = cellStartIndex; l<particle_grid.MATlength*particle_grid.MATwidth; l++) {
			        ...
			        density += (m+nv_m) * Smoothing_Kernel(q);
                    near_density += (m+nv_m) * Near_density_Kernel(q);
...
void addobjet(){
    Matrice nvmat;
    int n ;
    printf("quelle est la masse de ton objet");
    scanf("%d",&n);
    int i;
    if(objet-matlength>=0)
        i = matlength-1;
    else{
        i = matlength-2;
    }
    particle_grid.data[i][objet].rayon =  10 ;
    particle_grid.data[i][objet].position.x = (x_right-x_left)/2;
    particle_grid.data[i][objet].position.y = (y_down-y_up)/2;
    particle_grid.data[i][objet].position;
    particle_grid.data[i][objet].masse = n;
    //printf("\n%d",particle_grid.data[i][objet].masse);
    objet++;
}
