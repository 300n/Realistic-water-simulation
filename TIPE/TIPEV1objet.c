...
double2 Calculate_Density_objet(int k, int n)
{
    double density = 0, near_density = 0, q;
    Vect2D point = Vect2D_cpy(particle_grid.data[k][n].predicted_position);
    Vect2D SampleCell = Position_to_Cell(point);
...
void object_out_of_the_grid()
/*Détecte une sortie de l'écran*/
{
    for (int i = 0; i<objet_grid.MATlength; i++) { 
        for (int j = 0; j<objet_grid.MATwidth; j++) {
            O+=4;           
            if (objet_grid.data[i][j].position.x<x_left+pradius) {
                objet_grid.data[i][j].position.x = x_left+pradius;
                objet_grid.data[i][j].velocity.x *= DAMPING_COEFFICIENT;
            } else if (objet_grid.data[i][j].position.x>x_right-pradius) {    
                objet_grid.data[i][j].position.x = x_right-pradius;
                objet_grid.data[i][j].velocity.x *= DAMPING_COEFFICIENT;
            }
            if (objet_grid.data[i][j].position.y<y_up+pradius) {
                objet_grid.data[i][j].position.y = y_up+pradius;
                objet_grid.data[i][j].velocity.y *= DAMPING_COEFFICIENT;
            } else if (objet_grid.data[i][j].position.y>y_down-pradius) {
                objet_grid.data[i][j].position.y = y_down-pradius;
                objet_grid.data[i][j].velocity.y *= DAMPING_COEFFICIENT;
            }
        }
    }
...
void add_objet(){
    nb_objet++;
    Particule oui;
    oui.position.x=width/2;
    oui.position.y=height/2;
    objet_grid.data[0][nb_objet] = oui;
}
    ...
    if (!isPaused) {
	    ...
	    for (int i = 0 ; i < particle_grid.MATlength ; i++) {
            for (int j = 0 ; j < particle_grid.MATwidth ; j++) {
                ...
                if(i == 0 && j<nb_objet){
                    objet_grid.data[i][j].velocity.y += g * 20 / FPS;
                    //objet_grid.data[i][j].predicted_position.x = objet_grid.data[i][j].position.x +objet_grid.data[i][j].velocity.x*1 / FPS ;
                    //objet_grid.data[i][j].predicted_position.y = objet_grid.data[i][j].position.y +objet_grid.data[i][j].velocity.y*1 / FPS ;
                }
	    	}
	    }
        ...
        for (int i = 0 ; i < particle_grid.MATlength ; i++) {
            for (int j = 0 ; j < particle_grid.MATwidth ; j++) {
                ...
                objet_grid.data[i][j].density = densities.first_value;
                objet_grid.data[i][j].density = densities.second_value;
            }
        }
        ...
        for (int i = 0 ; i < particle_grid.MATlength ; i++) {
            for (int j = 0 ; j < particle_grid.MATwidth ; j++) {
                ...
                objet_grid.data[i][j].velocity.x+= pressure_acceleration.x * dt;
                objet_grid.data[i][j].velocity.y+= pressure_acceleration.y * dt;
        	}
        }   
        ...
        for (int i = 0; i < particle_grid.MATlength ; i++) {
            for (int j = 0; j < particle_grid.MATwidth ; j++) {
                ...
                objet_grid.data[i][j].velocity.x += viscosity_force.x * viscosity_strength * dt ;
                objet_grid.data[i][j].velocity.y += viscosity_force.y * viscosity_strength * dt ;
            }
        }
	    ...
	    for (int i = 0 ; i < particle_grid.MATlength ; i++) {
            for (int j = 0 ; j < particle_grid.MATwidth ; j++) {
                ...
                objet_grid.data[i][j].position.x += objet_grid.data[i][j].velocity.x*dt;
                objet_grid.data[i][j].position.y += objet_grid.data[i][j].velocity.y*dt;
                //printf("%d\n", objet_grid.data[i][j].position.x);
                //printf("%d\n", objet_grid.data[i][j].position.y);
	    	}
	    }

        particle_out_of_the_grid();
        object_out_of_the_grid();
        ...
        if(nb_objet!=0){
            for(int i = 0 ; i < nb_objet;i++){
                SDL_SetRenderDrawColor(renderer,255,255,255,SDL_ALPHA_OPAQUE);
                drawCircle(objet_grid.data[0][i].position.x,objet_grid.data[0][i].position.y,pradius+10);
            }
        }
